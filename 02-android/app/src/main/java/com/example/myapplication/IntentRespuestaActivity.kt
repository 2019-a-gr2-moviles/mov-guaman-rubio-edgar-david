...
class IntentRespuestaActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when(resultCode){
            RESULT_OK ->{Log.i("intent-respuesta","Lo logramos ${Activity.RESULT_OK}")

                when(requestCode){
                    304->{Log.i("intent-respuesta","Contacto llegó")
                        val uri = data?.data
            ...
            RESULT_CANCELED ->{Log.i("intent-respuesta","No eligió el contacto")}
        }
    }
}
